...
import 'dart:convert';
class Womenrankingplayer extends StatefulWidget{
  @override
  _WomenrankingplayerState createState() => _WomenrankingplayerState();
}

class _WomenrankingplayerState extends State<Womenrankingplayer> with TickerProviderStateMixin {
  ...
  void readData() {
    databaseReference.once().then((DataSnapshot snapshot) {
      ...
      T20.add(snapshot.value['Womens Ranking']['T20']);
      t20image.add(snapshot.value['MensRanking']['TestImages']);
      ODI.add(snapshot.value['Womens Ranking']['OneDay']);
      ODIimage.add(snapshot.value['Womens Ranking']['OneDay']['OneDayImages']);
      ...
      home: DefaultTabController(
        ...
        child: Scaffold(
          ...
          appBar: AppBar(
            ...
            title: const Text('ICC WOMENS (Rankings)'),
          ),
          body:  Stack(
            children: [
                ...
                child:isLoading==true?Center(child: CircularProgressIndicator(color: Colors.blue,),): TabBarView(
                  children: [
                   // TestRanking(testarray:testmatch,image:testmatchimages),
                    Odiranking(testarray:ODI,image:ODIimage,screen:"womens"),
                    T20ranking(testarray:T20,image:ODIimage,screen:"womens"),
                  ],
                ),
              ),
            ],
